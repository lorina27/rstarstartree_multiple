...
public final class RTree<T, S extends Geometry> {
    ...
    private String asString(RTree<T, S> tree, Node<T, S> node, String margin) {
       ...
       // s.append("Linear Sum: 1st dimension=");
        double sum = 0;
        double sum2=0;
       ...
       // sum2 += (list.get(i).geometry().mbr().min(0))*(list.get(i).geometry().mbr().min(0));}
        ...
        //s.append(" Square Sum: 1st dimension=");
        //s.append(sum2);
    ...
    public String asString3(RTree<T, S> tree, List<Entry<Object, Point>> listc) {
        if (!root.isPresent())
            ...
            return asString3(tree, root.get(), "", listc);
    }



    private String asString3(RTree<T, S> tree, Node<T, S> node, String margin, List<Entry<Object, Point>> listc) {
        ...
        if (!node.isLeaf()) {
            ...
            for (int i = 0; i < n.count(); i++) {
                Node<T, S> child = n.child(i);
                s.append(asString3(tree, child, margin + marginIncrement2, listc));
